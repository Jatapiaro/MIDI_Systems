...
void draw()
{
  ...
  for (int i=0;i<tuioObjectList.size();i++) {
     ...
     if ( tuioObjectList.get(i).getSymbolID() == 4 ) {
       float angle = tuioObjectList.get(i).getAngle();
       int ang = int(map(angle, 0, TWO_PI, 0, 127));
       bus.sendControllerChange(0, 1, ang);
     }
...
void addTuioObject(TuioObject tobj) {
  ...
  ArrayList<TuioObject> list = tuioClient.getTuioObjectList();
  for ( TuioObject to : list ) {
    if ( to.getSymbolID() == 1 ) {
      bus.sendNoteOn(0, 48, 70);
    }
  }
}
